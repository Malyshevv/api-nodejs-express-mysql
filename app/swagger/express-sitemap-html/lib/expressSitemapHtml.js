 ...
 */
function swagger(swDocument, app) {
    if(swaggerTag) return // We have already included swagger middlewares
    //swaggerTag = name
    app.use('/api-docs', swaggerUi.serve)
    app.get('/api-docs', swaggerUi.setup(swaggerJson(swDocument, app)))
}
 ...
 * @param {Express} app An express instance
 */
function swaggerJson(swDocument, app) {
    const routes = swaggerRoutes(app)
    const swaggerDocument = swDocument  
    routes
        .forEach(r => {
            if(!swaggerDocument.paths[r.path]) 
                swaggerDocument.paths[r.path] = {}
            const pathName = toUpFirstLetter(r.path.substring(1,r.path.length).split('/')[0]); 
            const pathMethod = swaggerPathMethod(pathName, r.bodyParams.length != 0)
            swaggerDocument.paths[r.path][r.method] = pathMethod
            r.routeParams.forEach(arg => { 
                pathMethod.parameters.push(swaggerPathParam(arg))
            })
            r.queryParams.forEach(arg => {
                pathMethod.parameters.push(swaggerQueryParam(arg))
            })
            r.bodyParams.forEach(arg => {
                pushBodyParam(pathMethod, 'x-www-form-urlencoded', arg)
                pushBodyParam(pathMethod, 'json', arg)
            })
            pathMethod.security = [{ "bearerAuth": [] }]
        })
